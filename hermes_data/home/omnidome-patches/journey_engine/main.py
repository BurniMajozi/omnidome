...
async def respond_to_offer(
    ...
    # 8. Bridge to lifecycle service — update customer lifecycle status
    try:
        import httpx
        lifecycle_url = os.getenv("LIFECYCLE_SERVICE_URL", "http://lifecycle:8018")
        outcome_map = {"accept": "accepted", "reject": "rejected"}
        lifecycle_payload = {
            "tenant_id": str(cancel_event.tenant_id),
            "customer_id": str(cancel_event.customer_id),
            "cancel_event_id": str(cancel_event.id),
            "outcome": outcome_map.get(data.decision, data.decision),
            "journey_id": str(cancel_event.matched_journey_id) if cancel_event.matched_journey_id else None,
            "offer_id": str(cancel_event.matched_offer_id) if cancel_event.matched_offer_id else None,
            "reason": cancel_event.customer_snapshot.get("churn_reason"),
        }
        async with httpx.AsyncClient(timeout=5) as client:
            await client.post(
                f"{lifecycle_url}/lifecycle/from-journey",
                json=lifecycle_payload,
            )
    except Exception:
        pass  # Don't fail the journey response if lifecycle is unavailable
