...
const ALLOW_DEV_HEADERS =
  process.env.NODE_ENV !== "production" || process.env.FINANCE_PROXY_ALLOW_DEV_HEADERS === "true"
...
async function proxy(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (ALLOW_DEV_HEADERS) {
    ...
    if (!headers.has("x-permissions")) headers.set("x-permissions", "finance.read,finance.write,finance.admin")
  }
