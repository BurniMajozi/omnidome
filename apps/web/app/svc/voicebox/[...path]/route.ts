...
async function proxy(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  try {
    ...
    if (err?.name === "AbortError") {
      return NextResponse.json({ error: "Voicebox request timed out — model may still be loading, try again shortly." }, { status: 504 })
    }
    return NextResponse.json({ error: "Voicebox service unreachable", details: String(err) }, { status: 502 })
  } finally {
    clearTimeout(timer)
  }
}
