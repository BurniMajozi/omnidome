...
async function proxy(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (ALLOW_DEV_HEADERS) {
    ...
    if (!headers.has("x-permissions")) headers.set("x-permissions", "marketing.read,marketing.write,marketing.admin")
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Marketing service unreachable", details: String(err) }, { status: 502 })
  }
}
