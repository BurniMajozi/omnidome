...
class VumatelAPI:
    ...
    async def place_order(
        ...
        if isinstance(customer_id_or_data, dict):
            ...
            address = address or str(customer_data.get("address", customer_data.get("installation_address", "")))
    ...
    # every adapter. Vumatel's partner API has no endpoint for them, so they
    # return an explicit FAILED result instead of raising AttributeError.

    async def cancel_order(self, order_id: str) -> dict:
        logger.warning("Vumatel cancel_order not supported by partner API: %s", order_id)
        return {"status": "FAILED", "error": "cancel_order not supported by Vumatel partner API"}

    async def check_coverage(self, latitude: str, longitude: str) -> dict:
        logger.warning("Vumatel check_coverage not supported (use check_availability): %s,%s", latitude, longitude)
        return {"fno": "vumatel", "available": False, "error": "GPS coverage check not supported; use check_availability with street address"}

    async def change_speed(self, fno_account_id: str, new_profile: str) -> dict:
        logger.warning("Vumatel change_speed not supported by partner API: %s", fno_account_id)
        return {"status": "FAILED", "error": "change_speed not supported by Vumatel partner API"}

    async def suspend_service(self, fno_account_id: str) -> dict:
        logger.warning("Vumatel suspend_service not supported by partner API: %s", fno_account_id)
        return {"status": "FAILED", "error": "suspend_service not supported by Vumatel partner API"}

    async def resume_service(self, fno_account_id: str) -> dict:
        logger.warning("Vumatel resume_service not supported by partner API: %s", fno_account_id)
        return {"status": "FAILED", "error": "resume_service not supported by Vumatel partner API"}

    async def report_fault(self, fno_account_id: str, description: str) -> dict:
        logger.warning("Vumatel report_fault not supported by partner API: %s", fno_account_id)
        return {"status": "FAILED", "error": "report_fault not supported by Vumatel partner API"}
